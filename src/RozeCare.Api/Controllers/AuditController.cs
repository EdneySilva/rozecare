...
public class AuditController : ControllerBase
{
    ...
    [HttpGet]
    [Authorize]
    public async Task<ActionResult<IReadOnlyList<AuditLogDto>>> Get([FromQuery] Guid? patientId, [FromQuery] Guid? actor, [FromQuery] DateTime? from, [FromQuery] DateTime? to)
    {
        ...
        var logs = await _mediator.Send(new GetAuditLogsQuery(patientId, actor, from, to));
        return Ok(logs);
    }
}
