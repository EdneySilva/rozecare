...
public class MedicationsController : ControllerBase
{
    ...
    [HttpGet]
    [Authorize]
    public async Task<ActionResult<IReadOnlyList<MedicationDto>>> Get(Guid patientId)
    {
        var items = await _mediator.Send(new GetMedicationsQuery(patientId));
        return Ok(items);
    }

    [HttpPost]
    [Authorize(Roles = "Clinician,OrgAdmin,Admin")]
    public async Task<ActionResult<Guid>> Post(Guid patientId, [FromBody] CreateMedicationRequest request)
    {
        var id = await _mediator.Send(new CreateMedicationCommand(patientId, request.Name, request.Dosage, request.StartDate, request.EndDate, request.PrescribedBy));
